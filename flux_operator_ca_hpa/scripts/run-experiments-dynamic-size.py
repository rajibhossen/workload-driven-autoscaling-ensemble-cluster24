...
def create_job_parameters():
    x = get_truncated_normal(mean=64, std=5, low=55, high=75)
    y = get_truncated_normal(mean=16, std=5, low=10, high=25)

    command = ["lmp", "-v", "x", str(int(x.rvs())), "-v", "y", str(int(y.rvs())), "-v", "z", str(int(y.rvs()))]
