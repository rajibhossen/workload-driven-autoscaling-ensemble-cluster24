...
# amg -P 16 8 4 -n 160 145 70
def create_job_parameters():
    x = get_truncated_normal(mean=150, std=5, low=140, high=160)
    y = get_truncated_normal(mean=135, std=5, low=125, high=145)
    z = get_truncated_normal(mean=60, std=5, low=50, high=70)

    command = ["amg", "-P 16 8 4", "-n", str(int(x.rvs())), str(int(y.rvs())), str(int(z.rvs()))]
...
def main():
    ...
    # python3 run-experiments.py --outdir /home/flux --workdir /opt/lammps/examples/reaxff/HNS --times 5 -N 8 --tasks 512 -in in.reaxc.hns -nocite --dry-run
